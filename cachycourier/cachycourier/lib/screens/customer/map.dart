...
import 'package:cachycourier/screens/customer/secrets.dart';
...
class _MapViewState extends State<MappPage> {
  ...
  _getCurrentLocation() async {
    ...
    _locationSubscription = locationTraker.onLocationChanged().listen((newLocalData) async {
      setState(() {
      print("get location");
      markers = {};
      polylines = {};
    ...
    print(zoomLevel);
    await _getAddress();
    await update_loc(_currentAddress); 
    await _calculateDistance(zoomLevel,centerBounds.latitude,centerBounds.longitude);
    // zoomLevel = await mapController.getZoomLevel();
    // print(zoomLevel);
    });
  }
  ...
  Future<String> start_dest(String id,String email) async {
    ...
    var url="http://"+DotEnv().env['ipadress']+":"+DotEnv().env['port']+"/addorder/map";
    print(url);
  ...
  void update_loc(String _currentAddress) async {
    print(DotEnv().env['ipadress']);
    var url="http://"+DotEnv().env['ipadress']+":"+DotEnv().env['port']+"/addorder/map1";
    print(url);
    http.Response resp = await http.post(url,body:{'email':email,'location':_currentAddress});  // 10.0.2.2 for emulator
    if (resp.statusCode == 200) {
      var jsonResponse = convert.jsonDecode(resp.body);
      print('$jsonResponse');
      print("updated loc");
  ...
  getRouteCoordinates(Position start, Position destination) async {
